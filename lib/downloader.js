...
(function() {
  ...
  Playlist = (function(superClass) {
    ...
    function Playlist() {
      this.generatePlaylistFile = bind(this.generatePlaylistFile, this);
    ...
    Playlist.prototype.generatePlaylistFile = function(filename, cb) {
      var dir, playlistFile;
      dir = Path.resolve("" + this.directory);
      playlistFile = dir + '/' + this.track.artist[0].name.replace(/\//g, ' - ') + '.m3u';
      fs.appendFile('.m3u', 'data to append', (function(_this) {
        return function(err) {
          if (err) {
            throw err;
          }
